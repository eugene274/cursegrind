...
public:
        ...
        if (ientry < entries.size ())
          {
            ...
            std::string position = name_repr == kObject
                                       ? entry->position->binary
            ...
            mvwprintw (window, iline, 1, "%s%s%s",
                       costs_stream.str ().c_str (), delim.c_str (),
                       name.c_str ());
            lastline = iline;
          }
        else
  ...
  shiftSelection (int pos)
  {
    ...
    if (entry_offset + selected_line >= parser->getEntries ().size ())
    ...
    else if (selected_line >= lastline)
      {
        ...
        if (entry_offset < parser->getEntries ().size () - 1)
          entry_offset++;
      }
  ...
  shiftPage (int d)
  {
    if (d > 0)
      {
        if (entry_offset + getNumberOfLines ()
            >= parser->getEntries ().size ())
          {
            /* do nothing */
          }
        else
  ...
  void
  shiftSelectedLineOffset (int pos)
  {
...
struct TreeView
{
  ...
  render ()
  {
    ...
    for (int iline = 1; iline < actual_height && inode < nodes.size ();
         ++iline, ++inode)
      {
        ...
        auto text_width
            = actual_width - left_offset - 1 - bullet_symbol.length ();
        ...
        mvwprintw (window, iline, left_offset + 1 + bullet_symbol.length (),
                   "%s", text.c_str ());
        wattron (window, COLOR_PAIR (1));
      }
  ...
  void
  SetItemView (const std::shared_ptr<ItemView> &item_view)
  {
...
private:
  ...
  collapse_selected ()
  {
    ...
    auto collapse_end = std::find_if (
        collapse_begin, end (nodes), [&current_node] (TreeNodePtr ptr) {
  ...
  TreeNodePtr
  makeCallNode (const CallgrindParser::EntryPtr &parent,
                const CallgrindParser::Call &call)
  {
    ...
    new_node->render_string = [this, parent, new_node, call] (int, int) {
      std::stringstream text_stream;
      text_stream << "> [calls=" << std::setprecision (2)
                  << double (call.ncalls) << "] ";
  ...
  makeEntryNode (const CallgrindParser::EntryPtr &entry)
  {
    ...
    new_node->render_string
        = [this, entry, new_node] (int, int) -> std::string {
      std::stringstream text_stream;
      if (costs_view == kAbsolute)
        {
  ...
  nextSelectable ()
  {
    ...
    if (next_selectable_node_it == end (nodes))
    ...
    else
      {
        ...
        selected_inode
            = std::distance (begin (nodes), next_selectable_node_it);
        nodes[selected_inode]->is_selected = true;
      }
    render ();
  }
  ...
  prevSelectable ()
  {
    ...
    auto current_node_rit
        = rbegin (nodes) + (nodes.size () - 1 - selected_inode);
    ...
    if (prev_selectable_node_it == rend (nodes))
    ...
    else
      {
        ...
        selected_inode
            = nodes.size ()
              - std::distance (rbegin (nodes), prev_selectable_node_it) - 1;
        nodes[selected_inode]->is_selected = true;
      }
  ...
  renderSearchForm ()
  {
    if (!search_form)
      {
        search_fields = { new_field (1 /* height */,
                                     getmaxx (window) - 11 - 2 /* width */,
                                     getmaxy (window) - 2 /* startpos y */,
                                     11 /* startposx */, 0, 1),
                          nullptr };
  ...
  doSearch ()
  {
    ...
    while (std::isspace (*buffer_begin))
      buffer_begin++;
    ...
    auto first_highlighted = std::find_if (
        begin (nodes), end (nodes), [] (const TreeNodePtr &node) {
  ...
  void
  setMessage (const std::string &message) const
  {
    if (item_view)
      {
