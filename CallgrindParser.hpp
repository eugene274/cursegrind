...
public:
  ...
  enum class PositionType : std::size_t
  {
    Cost = 0,
    Call = 1,
    FiFe = 2
  ...
  struct CallSpec
  {
    CallSpec (unsigned int ncalls,
              const std::vector<SubPosition> &sub_positions)
  ...
  struct Call
  {
    Call (unsigned int ncalls, std::vector<SubPosition> sub_positions,
        ...
        : ncalls (ncalls), sub_positions (std::move (sub_positions)),
    ...
    void
    addCost (const CostSpec &spec)
    {
      costs.emplace_back (spec);
    }
    std::vector<Cost>
    totalCosts () const
    {
  ...
  struct Position
  {
    ...
    void
    setPosition (const PositionSpec &spec)
    {
      if (spec.name == "ob")
        {
          binary = spec.value;
        ...
        if (std::optional<PositionSpec> cost_position; bool (
                cost_position = parsePositionLine (line, PositionType::Cost)))
          {
            ...
            /* populating position */
            {
              ...
              while (getline ()
                     && (cost_position
                         = parsePositionLine (line, PositionType::Cost)))
                {
                  current_position.setPosition (*cost_position);
                }
            ...
            while (true)
              {
                ...
                if (std::optional<PositionSpec> call_position_spec;
                          ...
                          = parsePositionLine (line, PositionType::Call)))
                  {
                    ...
                    call_entry->position
                        = get_position_from_cache (call_position);
                    /* here line is call line */
                    auto call_line = parseCallLine (line);
                    if (!call_line)
                      {
                        throw std::runtime_error (
                            "Expected call line at "
                            + std::to_string (current_line_number));
                      }
                    Call call (call_line->ncalls, call_line->sub_positions,
                               call_entry);
                    {
                      ...
                      if (!cost_spec)
                        {
                          throw std::runtime_error (
                              "Expected cost line after call at "
                              + std::to_string (current_line_number));
                        }
                      ...
                      while (getline ())
                        {
                          ...
                          /* fi/fe= */
                          else if (parsePositionLine (line,
                                                      PositionType::FiFe))
                            {
                              if (verbose_)
                                std::cout << "Ignore fife" << std::endl;
        ...
        /* header */
        else if (std::regex_search (line, match_results, re_positions_def))
          {
            for (size_t im = 1; im < match_results.size (); ++im)
              {
                positions_def.emplace_back (
                    match_results.str (im)); // TODO trim
              }
                ...
                if (*(entry1_call.entry->position) == *(entry2->position))
                  {
                    ...
                    auto this_caller = std::find_if (
                        begin (entry2->callers), end (entry2->callers),
...
private:
  ...
  NumberType
  parseNumber (const Iterator &begin, const Iterator &end)
  {
  ...
  SubPosition
  parseSubPosition (const Iterator &begin, const Iterator &end,
  ...
  bool
  parseEmptyLine (const std::string &empty_line)
  {
    static const std::regex re_empty ("^\\s*$");
    return std::regex_search (empty_line, re_empty);
  }

  std::optional<PositionSpec>
  parsePositionLine (const std::string &line, PositionType position_type)
  {
    ...
    if (std::regex_search (line, match_results, re_position_spec))
      {
        ...
        auto compression_index
            = match_results[2].matched ? parseNumber<unsigned int> (
                  match_results[2].first + 1, match_results[2].second - 1)
                                       : -1;
        ...
        if (!has_specified_name)
          {
            ...
            if (result == end (*cache))
              {
                throw std::runtime_error (
                    "Cannot find compression from the cache");
              }
            value = result->second;
          }
        else
        ...
        if (compression_index >= 0 && has_specified_name)
          {
            std::map<unsigned int, std::string> *cache
                = &symbol_compression_cache_;
            if (position == "fl" || position == "fe" || position == "fi")
              {
                cache = &file_compression_cache_;
              }
  ...
  std::optional<CostSpec>
  parseCostLine (const std::string &costs_line)
  {
  ...
  std::optional<CallSpec>
  parseCallLine (const std::string &line)
  {
...
private:
  void
  printTopEntries (std::ostream &os, unsigned int ne = 0) const
  {
    if (entries_.empty ())
      return;
